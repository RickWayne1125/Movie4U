...
client = pymongo.MongoClient('mongodb://localhost:27017')  # TODO:加用户，密码
...
def get_check_list():
    ...
    res = []
    for item in hot_movie_list:
        tmp = dict()
        tmp.update({"id": item['_id']})
        tmp.update({"name": item['title']})
        tmp.update({"tag": item['tag_name']})
        tmp.update({"url": item['url']})
        res.append(tmp)
    return res
    # return check_list
...
if __name__ == '__main__':
    ...
    # 获取一个电影列表中所有电影对应信息
    movie_info_list = get_movie_info_by_list([296, 467, 675, 691, 829, 1090, 1159, 1436, 1572, 2069])
